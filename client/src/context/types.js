export const ADD_ORGANIZATION = 'ADD_ORGANIZATION';
export const GET_ORGANIZATIONS = 'GET_ORGANIZATIONS';
export const GET_SHIFTS = 'GET_SHIFTS';
export const ADD_SHIFT = 'ADD_SHIFT';
export const ADD_SHIFT_JOINED = 'ADD_SHIFT_JOINED';
export const DELETE_ORGANIZATION = 'DELETE_ORGANIZATION';
export const DELETE_SHIFT = 'DELETE_SHIFT';
export const DELETE_SHIFT_LEAVE = 'DELETE_SHIFT_LEAVE';
export const DELETE_SHIFT_JOINED = 'DELETE_SHIFT_JOINED';
export const SET_CURRENT_ORGANIZATION = 'SET_CURRENT_ORGANIZATION';
export const SET_CURRENT_SHIFT = 'SET_CURRENT_SHIFT';
export const CLEAR_CURRENT_ORGANIZATION = 'CLEAR_CURRENT_ORGANIZATION';
export const CLEAR_CURRENT_SHIFT = 'CLEAR_CURRENT_SHIFT';
export const UPDATE_ORGANIZATION = 'UPDATE_ORGANIZATION';
export const UPDATE_SHIFT = 'UPDATE_SHIFT';
export const UPDATE_SHIFT_JOINED = 'UPDATE_SHIFT_JOINED';
export const FILTER_ORGANIZATIONS = 'FILTER_ORGANIZATIONS';
export const FILTER_SHIFTS = 'FILTER_SHIFTS';
export const FILTER_SHIFTS_JOINED = 'FILTER_SHIFTS_JOINED';
export const CLEAR_FILTER = 'CLEAR_FILTER';
export const CLEAR_ORGANIZATIONS = 'CLEAR_ORGANIZATIONS';
export const CLEAR_SHIFTS = 'CLEAR_SHIFTS';
export const SET_ALERT = 'SET_ALERT';
export const REMOVE_ALERT = 'REMOVE_ALERT';
export const REGISTER_SUCCESS = 'REGISTER_SUCCESS';
export const REGISTER_FAIL = 'REGISTER_FAIL';
export const USER_LOADED = 'USER_LOADED';
export const AUTH_ERROR = 'AUTH_ERROR';
export const LOGIN_SUCCESS = 'LOGIN_SUCCESS';
export const LOGIN_FAIL = 'LOGIN_FAIL';
export const LOGOUT = 'LOGOUT';
export const CLEAR_ERRORS = 'CLEAR_ERRORS';
export const GET_ORGANIZATION_TOKEN = 'GET_ORGANIZATION_TOKEN';
export const GET_SHIFTS_JOINED = 'GET_SHIFTS_JOINED';
export const GET_TOKEN_FAIL = 'GET_TOKEN_FAIL';
export const ORGANIZATION_LOADED = 'ORGANIZATION_LOADED';
export const ORGANIZATION_ERROR = 'ORGANIZATION_ERROR';
export const LEAVE_ORGANIZATION = 'LEAVE_ORGANIZATION';
export const ERROR = 'ERROR';
export const EDIT_USER = 'EDIT_USER';
export const LEAVE_EDIT_USER = 'LEAVE_EDIT_USER';
export const UPDATE_USER = 'UPDATE_USER';
export const GET_USERS = 'GET_USERS';
export const CLEAR_ORGANIZATION_ID = 'CLEAR_ORGANIZATION_ID';
export const JOIN_ORGANIZATION = 'JOIN_ORGANIZATION';